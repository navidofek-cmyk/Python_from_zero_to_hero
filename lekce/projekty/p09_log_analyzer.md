# Projekt 9: Log analyzer

Mini-projekt po **sekci IX (Konkurence)**. CLI nástroj pro analýzu log souborů — parsuje formátované logy, počítá úrovně a hledá top chyby.

**Použité koncepty:** `pathlib` (73), regex (79), `datetime` (77), `argparse` (81), `logging` (80), `Counter` (17), generátory (52).

## Jak spustit

```bash
# Demo mod (vytvoří testovací log):
python3 projekty/09_log_analyzer/analyzer.py --demo

# Vlastní soubor:
python3 projekty/09_log_analyzer/analyzer.py muj.log

# JSON výstup:
python3 projekty/09_log_analyzer/analyzer.py --demo --json
```

## Zdrojový kód — `analyzer.py`
